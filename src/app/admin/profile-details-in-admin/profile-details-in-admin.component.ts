...
import { faFolderOpen, faTimes, faCheck, faThList,faTrashAlt,faMobileAlt } from '@fortawesome/free-solid-svg-icons';
...
export class ProfileDetailsInAdminComponent implements OnInit {
  ...
  showRejectConfirmMessage = false;
  itemToEdit;
  ...
  showDeleteUserDialog(user){
    ...
    // this.route.navigate(["/dashboard/adminAllPartners"]);
  }
  ...
  showRejectConfirm(user){
    this.showRejectConfirmMessage = true;
    this.userToReject = user;

  }
  rejectAccount(profileId) {
    this.userServ.updateProfileByAdmin(profileId, { "status": "rejected" }, "admin", this.uuidValue, localStorage.getItem("auth")).subscribe(
      (Response: any) => {
        console.log(Response);
        this.route.navigate(["/dashboard"]);
  ...
  closeRejectConfirm(){
    this.showRejectConfirmMessage = false;
