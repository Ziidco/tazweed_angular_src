...
export class AllProjectsNewComponent implements OnInit {
  ...
  openProject(project,$event: MouseEvent): Promise<boolean> {
    // this.projectServ.selectedPoject.next(project);
    // this.route.navigate(["/dashboard/acceptPartnerByAdmin/" + project._id]);
    if ($event.ctrlKey || $event.metaKey) {
      const url = this.route.serializeUrl(
        this.route.createUrlTree(["/dashboard/acceptPartnerByAdmin/" + project._id])
      );
      window.open(url, '_blank');
    } else {
      return this.route.navigate(["/dashboard/acceptPartnerByAdmin/" + project._id]);
    }
  }
  ...
  getSingleBill(job) {
    ...
    console.log(this.projectToexportBeforePrint);
  ...
  export() {
    if(this.projectToexportBeforePrint.iconName == undefined){
      this.nameForExport = this.projectToexportBeforePrint.projectTitle;
    }
    else{
      this.nameForExport = this.projectToexportBeforePrint.projectTitle.value
    }
    //this.projectToexportBeforePrint.projectTitle
    this.exportAsService.save(this.exportAsConfig, this.nameForExport).subscribe(() => {
      // save started
    });
    this.showBillExport = false;
  ...
  searchWithFilter() {
    ...
    if (this.filteredEmail == "") {
      ...
      console.log(this.filteredEmail);
      
      // this.allJobsAdmin = this.allJobsAdmin.filter(
      //   res => {
      //     if((res.iconName).match(this.filteredEmail)){
      //       return res;
      //     }
      //   }
      // )

      this.projectServ.getAllProjectsForAdminByClientEmail(this.filteredEmail,"admin", this.uuidValue, localStorage.getItem("auth")).subscribe(
        (response: any) => {
  
          console.log("all admin jobs by email ---------------------- ");
