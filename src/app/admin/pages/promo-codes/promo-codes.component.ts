...
export class PromoCodesComponent implements OnInit {
  ...
  addCupon() {
    console.log("array length == " + this.productIdArray.length);
    (<FormArray>this.addCuponForm.get("productId")).clear();

   
    console.log(this.addCuponForm.get("productId").value);
    
    if(this.productIdArray.length==0){
      alert("اختار الخدمات المشمولة")

    }

    else{

      for(const it of this.productIdArray){
        const checkValue = new FormControl(it);
  ...
  onCheckboxChange(event, valueReal) {
    ...
    if (event.target.checked) {
      ...
      // (<FormArray>this.addCuponForm.get("productId")).push(checkValue);
      this.productIdArray.push(event.target.defaultValue)
    }
    else{
      // this.removeItemFromArray(this.addCuponForm.get("productId").value,event.target.defaultValue)
      this.removeItemFromArray(this.productIdArray,event.target.defaultValue)
    }
