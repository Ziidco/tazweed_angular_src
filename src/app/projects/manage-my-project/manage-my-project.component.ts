...
export class ManageMyProjectComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.projectSev.selectedPoject.subscribe(
      (response) => {
        ...
        this.editProjectForm.get("timePerDay").setValue(response.timePerDay);
  ...
  calculateCost() {
    ...
    let additionalVal = this.editProjectForm.get("addtionalAmount").value;
     this.finalTotalCost = +additionalVal + +this.projectCost;
  ...
  addAddition(val) {
    ...
    if (val < 0) {
      ...
      this.editProjectForm.get("addtionalAmount").patchValue(val);

      this.finalTotalCost = +val + this.projectCost;
      this.editProjectForm.get("totalCost").setValue(this.finalTotalCost);
